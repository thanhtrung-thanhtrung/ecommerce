...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 03, 2025 at 04:48 PM
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_TinhTonKhoRealTime` (`p_id_ChiTietSanPham` INT) RETURNS INT(11) DETERMINISTIC READS SQL DATA
BEGIN
    DECLARE v_SoLuongNhap INT DEFAULT 0;
    DECLARE v_SoLuongBan INT DEFAULT 0;
    DECLARE v_SoLuongHuy INT DEFAULT 0;
    -- Tổng số lượng đã nhập kho
    SELECT IFNULL(SUM(SoLuong),0) INTO v_SoLuongNhap FROM chitietphieunhap WHERE id_ChiTietSanPham = p_id_ChiTietSanPham;
    -- Tổng số lượng đã bán (đơn hàng trạng thái != huỷ)
    SELECT IFNULL(SUM(SoLuong),0) INTO v_SoLuongBan FROM chitietdonhang ctdh
      JOIN donhang dh ON ctdh.id_DonHang = dh.id
      WHERE ctdh.id_ChiTietSanPham = p_id_ChiTietSanPham AND dh.TrangThai NOT IN (5);
    -- Tổng số lượng hoàn lại do đơn bị huỷ (nếu có logic hoàn kho)
    -- SELECT IFNULL(SUM(SoLuong),0) INTO v_SoLuongHuy FROM ...
    RETURN GREATEST(0, v_SoLuongNhap - v_SoLuongBan + v_SoLuongHuy);
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `fn_CoTheBan` (`p_id_ChiTietSanPham` INT, `p_SoLuongCanBan` INT) RETURNS TINYINT(1) DETERMINISTIC READS SQL DATA
BEGIN
...
(48, 41, 369, 2, 2200000.00, 4400000.00),
(49, 42, 369, 4, 2200000.00, 8800000.00),
(50, 43, 380, 12, 2200000.00, 26400000.00);
...
(66, 41, 369, 12, 119999.00, 1439988.00),
(67, 41, 380, 12, 119999.00, 1439988.00),
(68, 41, 382, 1, 119999.00, 119999.00);
...
(380, 155, 7, 7, 'NIKE-VÀNG-42-7820'),
(381, 150, 10, 12, 'ADIDAS-XANHNAVY-45-1959'),
(382, 155, 8, 1, 'NIKE-ĐEN-43-5890');
...
(41, 'DH250703-41', NULL, '50LGrgOh5fEtmprkqqNf3Z2_LdFja7uQ', 'thanh trung', '0798355780', 'nha be á nha, Xã Dương Đức, Huyện Lạng Giang, Bắc Giang', 4400000.00, 50000.00, 0.00, 440000050000.00, NULL, 'n', 3, 4, 4, 0, NULL, '2025-07-03 20:02:05', '2025-07-03 20:22:42', 'thanhtrung3010xsw@gmail.comk', NULL),
(42, 'DH250703-42', 50, NULL, 'chun chunadđghjkg', '0798355784', 'nhà bè than yêuuuuuu, Xã Phú Trung, Huyện Phú Riềng, Bình Phước', 8800000.00, 50000.00, 0.00, 880000050000.00, NULL, NULL, 3, 4, 4, 0, NULL, '2025-07-03 20:13:54', '2025-07-03 20:43:14', 'thanhtrung3010xsw@gmail.com', NULL),
(43, 'DH250703-43', 50, NULL, 'chun chunadđghjkg', '0798355784', 'nhà bè than yuzu, Xã Lai Hưng, Huyện Bàu Bàng, Bình Dương', 26400000.00, 50000.00, 0.00, 2640000050000.00, NULL, NULL, 3, 4, 1, 0, NULL, '2025-07-03 20:58:40', '2025-07-03 20:58:40', 'thanhtrung3010xsw@gmail.com', NULL);
...
(41, 'PN-250703-001', 5, 1, 0.00, 'n', '2025-07-03 20:00:08', 2);
...
(155, 'Giày Thể Thao Nike Air Max 2023', 'Giày thể thao cao cấp với công nghệ đệm khí', 'Giày thể thao Nike Air Max 2023 được thiết kế với công nghệ đệm khí tiên tiến, mang lại cảm giác thoải mái và hỗ trợ tốt cho chân khi vận động. Phần upper làm từ vải mesh thoáng khí giúp giảm thiểu mồ hôi và mùi hôi.', '{\"ChatLieu\":\"Vải mesh, cao su\",\"KieuGiay\":\"Thể thao\",\"XuatXu\":\"Chính hãng\"}', 2500000.00, 2200000.00, 7, 1, 53, 1, '{\"anhChinh\":\"https://res.cloudinary.com/db7jn3ooa/image/upload/v1750488275/shoes_shop/products/avprg3oqnvehmow0t5k9.jpg\",\"anhChinh_public_id\":\"shoes_shop/products/avprg3oqnvehmow0t5k9\",\"anhPhu\":[\"https://res.cloudinary.com/db7jn3ooa/image/upload/v1750488279/shoes_shop/products/hdwcylnbfwvvuqkyff6h.jpg\",\"https://res.cloudinary.com/db7jn3ooa/image/upload/v1750488281/shoes_shop/products/twj5iiw4kcweksgomob3.jpg\"],\"anhPhu_public_ids\":[\"shoes_shop/products/hdwcylnbfwvvuqkyff6h\",\"shoes_shop/products/twj5iiw4kcweksgomob3\"]}', 1, '2025-06-21 13:28:43', '2025-07-03 20:22:36'),
