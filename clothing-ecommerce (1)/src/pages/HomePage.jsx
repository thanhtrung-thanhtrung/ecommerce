              ...
              >
                <div className="aspect-w-4 aspect-h-3">
                  <img
                    src={category.image}
                    alt={category.name}
                    className="w-full h-40 sm:h-64 object-cover group-hover:scale-50 transition-transform duration-300"
                  />
                </div>
              ...
              {products.slice(0, 8).map((product) => (
                <div key={product.id} className="card card-hover">
                  <Link to={`/products/${product.id}`}>
                    <div className=" group  aspect-w-1 aspect-h-1 mb-4">

                      <img
                        src={getProductImage(product)}
                        alt={product.Ten}
                        className="  w-64 h-64 right-11 object-cover rounded-lg transition-transform duration-300 group-hover:scale-150"
                      />
                    </div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {brands.filter(brand => brand.TrangThai === 1).map((brand) => (
              <div
                key={brand.id}
