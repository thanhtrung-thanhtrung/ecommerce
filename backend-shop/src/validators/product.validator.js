...
const createProductAdminValidator = [
  ...
  body("MoTaChiTiet")
    ...
    .trim()
    .isLength({ min: 10 })
    .withMessage("Mô tả chi tiết phải có ít nhất 10 ký tự"),
  ...
  body("bienThe")
    .notEmpty()
    .withMessage("Biến thể sản phẩm không được để trống")
    .custom((value, { req }) => {
      try {
        // Nếu đã là mảng, kiểm tra độ dài
        if (Array.isArray(value)) {
  ...
  body("bienThe.*.id_KichCo").custom((value, { req }) => {
    ...
    if (Array.isArray(bienThe)) {
      for (const variant of bienThe) {
        ...
        if (isNaN(parseInt(variant.id_KichCo))) {
          throw new Error("ID kích cỡ không hợp lệ");
        }
      }
    }
    return true;
  ...
  body("bienThe.*.SoLuong").custom((value, { req }) => {
    const bienThe = req.body.bienThe;
    if (!bienThe) return true;

    if (Array.isArray(bienThe)) {
      for (const variant of bienThe) {
        if (variant.SoLuong === undefined || variant.SoLuong === null) {
          throw new Error("Số lượng không được để trống");
        }
        if (isNaN(parseInt(variant.SoLuong)) || parseInt(variant.SoLuong) < 0) {
